...
export class ScoreManager extends EventEmitter {
  ...
  public calculateScore(
    ...
    const accuracyScore = this.calculateAccuracyScore(example, finalAttempt, constraintBoxes?.[attempts.length - 1]);
  ...
  private calculateStrokesScore(example: DrawingData, attempt: DrawingData): number {
    ...
    const strokeCountScore = maxStrokes > 0 
      ? Math.max(0, 1 - (countDifference / maxStrokes))
      : 1;
