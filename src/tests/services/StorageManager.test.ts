...
import { Exercise, DrawingData } from '../../types/Exercise';
...
describe('StorageManager', () => {
  ...
  it('should handle errors when saving exercise', async () => {
    ...
    await expect(storageManager.saveExercise(mockExercise))
      .rejects.toThrow('Failed to save exercise');
  });
  ...
  it('should handle nonexistent exercise updates', async () => {
    ...
    await expect(storageManager.updateExercise(nonexistentExercise))
      .rejects.toThrow('Exercise with ID nonexistent not found');
  });
  ...
  it('should handle exercise result saving for nonexistent exercise', async () => {
    ...
    await expect(storageManager.saveExerciseResult(mockExercise, scoreResult))
      .rejects.toThrow('Exercise with ID 1 not found');
  });
